...
class VideoThread:
    def __init__(self, videos, labels, model, path_var, p_container, c_container, fps=8):
        ...
        if os.path.exists('datasets/stills'):
            self.p_video_count = int(len([f for f in glob.glob('datasets/stills/MMISB Cropped/**/**/**/*.png')]) / 15)
            print(f"Starting on video {self.p_video_count}")
        else:
    ...
    def video_cropping_thread(self):
        end_count = len(self.videos)
        video_order = random.sample(range(len(self.videos)), end_count)
        for i in video_order:
            self.dataset_dir = os.path.join('datasets/stills/MMISB Cropped', pathlib.Path(self.videos[i]).parts[-3])
            if not os.path.exists(self.dataset_dir):
                os.mkdir(self.dataset_dir)
            self.event_dir = os.path.join(self.dataset_dir, pathlib.Path(self.videos[i]).parts[-2])
            if not os.path.exists(self.event_dir):
                os.mkdir(self.event_dir)
            self.video_dir = os.path.join(self.event_dir, pathlib.Path(self.videos[i]).stem)
            if not os.path.exists(self.video_dir):
                os.mkdir(self.video_dir)
                shutil.copy(r"C:\GitHub\Keypoint-LSTM\datasets\classes.txt", os.path.join(self.video_dir, 'classes.txt'))
            else:
                continue
            self.path_var.set(pathlib.Path(str(self.videos[i])).name)
            print(f"Loading video {i} of {end_count}: {self.videos[i]}")
    ...
    def load_video(self, path, max_frames=0):
        ...
        for frame in frame_data.iter_data():
            try:
                frame_path = os.path.join(self.video_dir, str(len([f for f in glob.glob(f'{self.video_dir}/*.png')])) + '.png')
                bb_path = frame_path[:-3] + 'txt'
                if self.last_image is not None:
                    if (self.last_image == frame).all():
                        continue
                # if os.path.exists(f'{self.video_dir}/{self.p_video_count}_{self.c_accepted+1}.txt'):
                #     self.c_accepted += 1
                #     self.p_accepted += 1
                #     continue
                ...
                for xyxy, xywh, preds in zip(img_result.xyxy[0], img_result.xywhn[0], img_result.pred[0]):
                    if preds[5] in classes and preds[4] > 0.6:
                        x1 = int(xyxy[0].item())
                        y1 = int(xyxy[1].item())
                        x2 = int(xyxy[2].item())
                        y2 = int(xyxy[3].item())
                ...
                # self.load_p_selection()
                # self.load_c_selection()
                imageio.imwrite(frame_path, self.last_image)
...
root_filepath = r"E:\Thesis Results\Datasets\MMISB Frequency"
new_files = [f for f in glob.glob(f'{root_filepath}/**/**/*.mp4')]

# root_filepath = r"C:\Users\wsarc\OneDrive - University of Nebraska Medical Center\UNMC Data\Problematic Behavior Recognition\Final Preliminary Dataset\2. event folder"
# old_files = [f for f in glob.glob(f'{root_filepath}/**/*.mp4')]

# root_filepath = r"E:\Thesis Results\Datasets\MMISB Cropped Duration"
# dur_files = [f for f in glob.glob(f'{root_filepath}/**/*.mp4')]

# old_files = [x for x in old_files if '2021-12-07 10111_1_0_9569_9585_805' in x or '2021-12-07 10111_1_26_9567_9583_804' in x]
all_videos = [*new_files]
...
detection_model = torch.hub.load('ultralytics/yolov5',
                                 'custom',
                                 path=r"C:\GitHub\yolov5\runs\train\exp21\weights\best.pt",
                                 force_reload=True)

video_object = VideoThread(all_videos, [], detection_model, video_label_var, patient_container,
                           clinician_container)
